...
extern void pre_fmg(vector<mg_grid>&, pbcs&); 	   //Function for FMG 
...
//For the Multigrid Preconditioned Conjugate Gradient Method 

extern vec mulvec(vec,int);  
extern vec precond(vector<mg_grid>&, vec, vec, pbcs&); 
extern void adjust_precond(vector<mg_grid>& ,vec &);
extern vec mg_post_precond(vector<mg_grid>&); 
extern vec error_from_res(vec &);

extern void mg_conjugate_gradient(vector<mg_grid>& , vector<fval>& , pbcs&); 
extern void mg_conjugate_gradient(vector<mg_grid>& ,int ,int ,int ,int );
extern void mg_pre_conjugate_gradient(vector<mg_grid>& , vector<fval>& , pbcs&); 
extern void mg_pre_initialize(vector<mg_grid>& ,vec &, vec &);
