...
const int nx=nx_per+1, ny=ny_per+1, nz=nz_per+1;  //No. of spacings in X, Y and Z directions for the computational domain
...
const double tot_time = 50.0; //Some-non-dimenisonal time 
...
const int kx_min = -(nx/2)+1,kx_max=nx/2;
const int ky_min = -(ny/2)+1,ky_max=ny/2;
...
const int mg_levels_x = log2(nx_per) - log2(min_stencil_req) + 1; // Calculating the no. of multi grid levels needed in x-direction 
const int mg_levels_y = log2(ny_per) - log2(min_stencil_req) + 1; // Calculating the no. of multi grid levels needed in y-direction 

const int mg_levels = min(mg_levels_x,mg_levels_y); 

const int mg_full_x = log2(nx_per); 
const int mg_full_y = log2(ny_per); 
